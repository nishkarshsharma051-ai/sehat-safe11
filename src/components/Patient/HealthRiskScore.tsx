...
function calculateRisk(f: RiskFactors): { score: number; level: 'low' | 'moderate' | 'high'; details: string[] } {
    let score = 0;
    const details: string[] = [];

    // Age factor
    if (f.age > 60) { score += 25; details.push('Age above 60 increases cardiovascular risk'); }
    else if (f.age > 45) { score += 15; details.push('Age above 45 — moderate age risk'); }
    else if (f.age > 30) { score += 5; }

    // BMI factor
    if (f.bmi > 30) { score += 25; details.push('BMI indicates obesity — high metabolic risk'); }
    else if (f.bmi > 25) { score += 15; details.push('BMI indicates overweight — moderate risk'); }
    else if (f.bmi < 18.5) { score += 10; details.push('BMI indicates underweight — nutritional concern'); }

    // BP
    if (f.bpSystolic >= 140 || f.bpDiastolic >= 90) { score += 25; details.push('Blood pressure is high (Stage 2 hypertension)'); }
    else if (f.bpSystolic >= 130 || f.bpDiastolic >= 80) { score += 15; details.push('Blood pressure is elevated (Stage 1)'); }

    // Sugar
    if (f.sugarLevel > 200) { score += 25; details.push('Blood sugar is very high — diabetic range'); }
    else if (f.sugarLevel > 140) { score += 15; details.push('Blood sugar is elevated — pre-diabetic range'); }
...
export default function HealthRiskScore() {
    ...
    const handleCalculate = () => {
        ...
        setResult(calculateRisk({ age, weight, bpSystolic, bpDiastolic, sugarLevel, bmi }));
    };

    const colorMap = { low: 'green', moderate: 'yellow', high: 'red' };
    const iconMap = { low: TrendingDown, moderate: Minus, high: TrendingUp };
    const labelMap = { low: 'Low Risk', moderate: 'Moderate Risk', high: 'High Risk' };
        ...
        <div className="space-y-6">
            <div className="glass-card p-6">
                <div className="flex items-center space-x-3 mb-6">
                    ...
                    <div>
                        <h3 className="text-lg font-bold text-gray-800">Health Risk Assessment</h3>
                        <p className="text-sm text-gray-500">Enter your vitals to calculate your health risk score</p>
                    </div>
                </div>

                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
                    {[
                        { label: 'Age', key: 'age', icon: Heart, placeholder: '30', unit: 'years' },
                        { label: 'Weight', key: 'weight', icon: Scale, placeholder: '70', unit: 'kg' },
                        { label: 'Height', key: 'height', icon: Activity, placeholder: '170', unit: 'cm' },
                        { label: 'BP Systolic', key: 'bpSystolic', icon: Heart, placeholder: '120', unit: 'mmHg' },
                        { label: 'BP Diastolic', key: 'bpDiastolic', icon: Heart, placeholder: '80', unit: 'mmHg' },
                        { label: 'Sugar Level', key: 'sugarLevel', icon: Droplet, placeholder: '100', unit: 'mg/dL' },
                    ].map(field => (
                        <div key={field.key}>
                ...
                <div className="glass-card p-6">
                    ...
                    {form.weight && form.height && (
                        <div className="mb-4 p-3 bg-blue-50 rounded-xl text-sm text-center">
                            <p className="text-gray-600">Your BMI: <strong className="text-blue-700">{(parseFloat(form.weight) / ((parseFloat(form.height) / 100) ** 2)).toFixed(1)}</strong></p>
                        </div>
                    )}
                    ...
                    {result.level === 'low' && (
                        <div className="mt-4 p-3 bg-green-50 rounded-xl text-sm text-green-700">
                            ✅ Great news! Your health indicators are within normal ranges. Keep up the healthy lifestyle!
                        </div>
                    )}

                    <p className="text-xs text-gray-400 mt-4 text-center">
                        ⚠️ This is an approximate assessment. Always consult a healthcare professional for accurate diagnosis.
                    </p>
                </div>
            )}
        </div>
