...
export default function InsuranceTracker() {
    ...
    const deleteRecord = async (id: string) => {
        if (!confirm(t('Delete this insurance record?', 'इस बीमा रिकॉर्ड को हटाएं?'))) return;
            ...
            <div className="glass-card p-6">
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            <h3 className="text-lg font-bold text-gray-800">{t('Insurance Tracker', 'बीमा ट्रैकर')}</h3>
                            <p className="text-sm text-gray-500">{t('Manage your health insurance policies', 'अपने स्वास्थ्य बीमा पॉलिसियों का प्रबंधन करें')}</p>
                        </div>
                    </div>
                    <button onClick={() => setShowForm(!showForm)}
                        ...
                        <Plus className="w-4 h-4" /><span>{t('Add Policy', 'पॉलिसी जोड़ें')}</span>
                    </button>
                </div>
                    ...
                    <div className="mb-6 p-4 bg-blue-50/50 rounded-xl space-y-3">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <input value={form.provider} onChange={e => setForm({ ...form, provider: e.target.value })}
                                className="px-3 py-2 rounded-lg glass-input text-sm" placeholder={t('Insurance Provider', 'बीमा प्रदाता')} />
                            <input value={form.policy_number} onChange={e => setForm({ ...form, policy_number: e.target.value })}
                                className="px-3 py-2 rounded-lg glass-input text-sm" placeholder={t('Policy Number', 'पॉलिसी नंबर')} />
                            <select value={form.coverage_type} onChange={e => setForm({ ...form, coverage_type: e.target.value })}
                                ...
                                <option value="">{t('Coverage Type', 'कवरेज प्रकार')}</option>
                                <option value="Health">{t('Health', 'स्वास्थ्य')}</option>
                                <option value="Life">{t('Life', 'जीवन')}</option>
                                <option value="Accident">{t('Accident', 'दुर्घटना')}</option>
                                <option value="Critical Illness">{t('Critical Illness', 'गंभीर बीमारी')}</option>
                            </select>
                            <input type="date" value={form.expiry_date} onChange={e => setForm({ ...form, expiry_date: e.target.value })}
                                className="px-3 py-2 rounded-lg glass-input text-sm" />
                            <input type="number" value={form.premium} onChange={e => setForm({ ...form, premium: e.target.value })}
                                className="px-3 py-2 rounded-lg glass-input text-sm" placeholder={t('Annual Premium (₹)', 'वार्षिक प्रीमियम (₹)')} />
                        </div>
                        <div className="flex justify-end space-x-2">
                            <button onClick={() => setShowForm(false)} className="px-4 py-2 text-gray-600 text-sm">{t('Cancel', 'रद्द करें')}</button>
                            <button onClick={addRecord} className="px-4 py-2 bg-emerald-500 text-white rounded-lg text-sm hover:bg-emerald-600 transition-all">{t('Save', 'सहेजें')}</button>
                        </div>
                    </div>
                )}
                ...
                {records.length === 0 ? (
                    <div className="text-center py-12">
                        ...
                        <h4 className="text-lg font-bold text-gray-600 mb-2">{t('No Insurance Policies', 'कोई बीमा पॉलिसी नहीं')}</h4>
                        <p className="text-gray-500 text-sm">{t('Add your first insurance policy above', 'ऊपर अपनी पहली बीमा पॉलिसी जोड़ें')}</p>
                    </div>
                ) : (
                                ...
                                <div key={record.id} className={`p-5 rounded-xl border ${expired ? 'bg-red-50/50 border-red-200' : expiringSoon ? 'bg-yellow-50/50 border-yellow-200' : 'bg-white/40 border-gray-100'}`}>
                                    <div className="flex items-start justify-between mb-3">
                                        ...
                                        <div className="flex items-center space-x-2">
                                            {expired && <span className="px-2 py-0.5 bg-red-100 text-red-600 rounded-full text-xs font-bold">{t('Expired', 'समाप्त')}</span>}
                                            {expiringSoon && <span className="px-2 py-0.5 bg-yellow-100 text-yellow-700 rounded-full text-xs font-bold flex items-center"><AlertTriangle className="w-3 h-3 mr-1" />{days} {t('days left', 'दिन शेष')}</span>}
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-3">
                                        {record.expiry_date && (
                                            <div className="flex items-center text-sm text-gray-600">
                                                ...
                                                {t('Expires:', 'समाप्त होता है:')} {new Date(record.expiry_date).toLocaleDateString(t('en-IN', 'hi-IN'), { day: 'numeric', month: 'short', year: 'numeric' })}
                                            </div>
                                        )}
                                        {record.premium && (
                                            <div className="flex items-center text-sm text-gray-600">
                                                <DollarSign className="w-4 h-4 mr-1 text-gray-400" />
                                                {t('Premium:', 'प्रीमियम:')} ₹{record.premium.toLocaleString(t('en-IN', 'hi-IN'))}/{t('yr', 'वर्ष')}
                                            </div>
                                        )}
                                        <div className="flex items-center text-sm text-gray-600">
                                            <FileText className="w-4 h-4 mr-1 text-gray-400" />
                                            {t('Claims:', 'दावे:')} {record.claims.length}
                                        </div>
                                    </div>

                                    {record.claims.length > 0 && (
                                        <div className="mb-3">
                                            <p className="text-xs font-semibold text-gray-500 mb-2">{t('Claim History:', 'दावे का इतिहास:')}</p>
                                            <div className="space-y-1">
                                                {record.claims.map(claim => (
                                                    <div key={claim.id} className="flex items-center justify-between text-sm p-2 bg-white/50 rounded-lg">
                                                        ...
                                                        <div className="flex items-center space-x-2">
                                                            <span className="font-medium">₹{claim.amount.toLocaleString(t('en-IN', 'hi-IN'))}</span>
                                                            <span className={`px-1.5 py-0.5 rounded text-xs font-medium ${claim.status === 'approved' ? 'bg-green-100 text-green-700' :
                                                                claim.status === 'rejected' ? 'bg-red-100 text-red-700' :
                                                                    'bg-yellow-100 text-yellow-700'}`}>{claim.status === 'approved' ? t('approved', 'अनुमोदित') : claim.status === 'rejected' ? t('rejected', 'अस्वीकार') : t('pending', 'लंबित')}</span>
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
                                        ...
                                        <div className="p-3 bg-blue-50/50 rounded-lg space-y-2">
                                            <div className="grid grid-cols-3 gap-2">
                                                <input value={claimForm.description} onChange={e => setClaimForm({ ...claimForm, description: e.target.value })}
                                                    className="col-span-2 px-2 py-1.5 rounded glass-input text-xs" placeholder={t('Claim description', 'दावे का विवरण')} />
                                                <input type="number" value={claimForm.amount} onChange={e => setClaimForm({ ...claimForm, amount: e.target.value })}
                                                    className="px-2 py-1.5 rounded glass-input text-xs" placeholder={t('Amount (₹)', 'राशि (₹)')} />
                                            </div>
                                            <div className="flex justify-end space-x-2">
                                                <button onClick={() => setShowClaimForm(null)} className="text-xs text-gray-500">{t('Cancel', 'रद्द करें')}</button>
                                                <button onClick={() => addClaim(record.id)} className="px-3 py-1 bg-blue-500 text-white rounded text-xs">{t('Add Claim', 'दावा जोड़ें')}</button>
                                            </div>
                                        </div>
                                    ) : (
                                        <button onClick={() => setShowClaimForm(record.id)}
                                            className="text-sm text-blue-600 hover:text-blue-700 font-medium flex items-center space-x-1">
                                            <Plus className="w-3 h-3" /><span>{t('Add Claim', 'दावा जोड़ें')}</span>
                                        </button>
                                    )}
                                </div>
                            );
